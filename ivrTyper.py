...
def ivrTyper(args, time):
    ...
    if args.read1 is not None and args.read2 is not None:
        ...
        sample_data, searched_fastq_files = getListIDs(workdir,
                                                       args.listIDs.name if args.listIDs is not None else None,
                                                       args.taxon)
        ...
        if not searched_fastq_files:
            ...
            if out[0]:
                ...
                success = runTyper.alignSamples(sample, files, reference, args.threads, workdir, script_path,
                                                args.keepFiles, args.minCoverage, args.proportionCutOff, time,
                                                args.greaterThan, args.reportFile)
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='ivrTyper.py', description='Reads mapping against pneumococcal ivr '
                                                                     'locus for typing.',
    ...
    parser_optional_general.add_argument('-1', '--read1', metavar='sample_1.fastq.gz',
                                         help='Forward read file (fastq.gz or fq.gz and pair-end direction coded as '
                                              '_R1_001 / _R2_001 or _1 / _2)')
    parser_optional_general.add_argument('-2', '--read2', metavar='sample_2.fastq.gz',
                                         help='Forward read file (fastq.gz or fq.gz and pair-end direction coded '
                                              'as _R1_001 / _R2_001 or _1 / _2)')
    parser_optional_general.add_argument('-j', '--threads', type=int, metavar='N', help='Number of threads to use',
                                         required=False, default=1)
    parser_optional_general.add_argument('-u', '--skipProvidedSoftware', action='store_true',
                                         help='Do not use provided software',
    ...
    parser_optional_ivrTyper.add_argument('-c', '--proportionCutOff', type=float, metavar='N',
                                          help='Proportion cut off for the module 1.x to be chosen as target',
    ...
    parser_optional_download.add_argument('-pm', '--platformModel', type=str, metavar='HiSeq', help='Filter '
                                          'download by the model of the Illumina machine.',
                                          choices=['HiSeq', 'MiSeq', 'None'], required=False, default=None)
    parser_optional_download.add_argument('-ls', '--librarySource', type=str, metavar='GENOMIC', help='Filter '
                                          'download by library source (available options: %(choices)s)',
                                          choices=['GENOMIC', 'ALL'],
    ...
    print('\n {} samples out of {} ran successfully'.format(str(number_samples_successfully),
                                                            str(samples_total_number)))
